...
func _load_enemy_config() -> void:
	...
	max_health = config.get("max_health", 35)
	move_speed = config.get("move_speed", 180.0)
	detection_range = config.get("detection_range", 500.0)
	attack_range = config.get("attack_range", 80.0)
	attack_damage = config.get("attack_damage", 15)
	attack_cooldown = config.get("attack_cooldown", 1.0)
	
	fly_speed = config.get("fly_speed", 180.0)
	hover_amplitude = config.get("hover_amplitude", 20.0)
	hover_frequency = config.get("hover_frequency", 2.0)
	dive_speed = config.get("dive_speed", 400.0)
	dive_range = config.get("dive_range", 200.0)
