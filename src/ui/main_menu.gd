...
@export var character_config_button: Button
...
@export var character_config_panel: Control
...
func _ready() -> void:
	...
	if character_config_button:
		character_config_button.pressed.connect(_on_character_config_pressed)
	if credits_button:
	...
	if version_label:
		version_label.text = (
			"v" + ProjectSettings.get_setting("application/config/version", "0.1.0")
		)
...
func _auto_get_node_references() -> void:
	...
	if not character_config_button:
		character_config_button = get_node_or_null("MainPanel/CharacterConfigButton")
	...
	if not character_config_panel:
		character_config_panel = get_node_or_null("CharacterConfigPanel")
...
func _connect_back_buttons() -> void:
	...
	# CharacterConfigPanel 使用 close_requested 信号
	if character_config_panel and character_config_panel.has_signal("close_requested"):
		character_config_panel.close_requested.connect(_on_back_to_main)
...
func _on_character_config_pressed() -> void:
	_show_panel(character_config_panel)
...
func _show_panel(panel: Control) -> void:
	if current_panel:
		if current_panel.has_method("hide_panel"):
...
func _show_delete_confirmation(slot_index: int) -> void:
	...
	dialog.dialog_text = LocalizationManager.call(
		"tr", "ui.main_menu.dialog.delete_save.text", {"slot": slot_index + 1}
	)
...
func _show_new_game_slot_confirmation() -> void:
	...
	dialog.dialog_text = LocalizationManager.call(
		"tr", "ui.main_menu.dialog.new_game_slot.text", {"slot": _pending_new_game_slot + 1}
	)
...
func _apply_localized_texts() -> void:
	...
	if character_config_button:
		character_config_button.text = _tr("ui.main_menu.button.character_config")
	if credits_button:
