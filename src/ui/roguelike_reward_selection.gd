...
func _resolve_node_references() -> void:
	...
	if option_1_button == null:
		option_1_button = (
			get_node_or_null("CenterContainer/VBoxContainer/Options/Option1Button") as Button
		)
	if option_2_button == null:
		option_2_button = (
			get_node_or_null("CenterContainer/VBoxContainer/Options/Option2Button") as Button
		)
	if option_3_button == null:
		option_3_button = (
			get_node_or_null("CenterContainer/VBoxContainer/Options/Option3Button") as Button
		)
...
func show_rewards(options: Array[Dictionary]) -> void:
	if options.size() != _option_buttons.size():
		push_warning(
			"RoguelikeRewardSelection.show_rewards expected 3 options, got %d" % options.size()
		)
...
func _bind_options(options: Array[Dictionary]) -> void:
	for index in range(_option_buttons.size()):
		...
		var blessing_id := String(
			option.get("id", option.get("blessing_id", option.get("key", "")))
		)
...
func _resolve_option_title(option: Dictionary, blessing_id: String) -> String:
	...
	return String(
		option.get(
			"title",
			(
				LocalizationManager.tr("ui.roguelike_reward_selection.unknown_blessing")
				if LocalizationManager
				else "Unknown Blessing"
			)
		)
	)
...
func _resolve_option_description(option: Dictionary, blessing_id: String) -> String:
	...
	return (
		LocalizationManager.tr("ui.roguelike_reward_selection.unknown_description")
		if LocalizationManager
		else "Description unavailable"
	)


func _translate_option_key(
	option: Dictionary, explicit_key_field: String, blessing_id: String, suffix: String
) -> String:
	if not LocalizationManager:
		return ""
